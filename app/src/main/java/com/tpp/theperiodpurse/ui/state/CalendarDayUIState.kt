...
data class CalendarDayUIState(
    ...
    private val exerciseLength =
        if (exerciseLengthString != "") LocalDateTime.parse(exerciseLengthString, formatter) else null
    ...
    val sleep = if (sleepString != "")  LocalDateTime.parse(sleepString, formatter) else null
    val sleepScore =
